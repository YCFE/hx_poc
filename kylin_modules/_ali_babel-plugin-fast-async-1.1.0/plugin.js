...
var hoistHelper = require('./hoist.js');
var hoistFunction = hoistHelper.hoistFunction;
...
module.exports = function (babel) {
    ...
    var warnSwitchCaseVisitor = {
        SwitchStatement: function(path, state) {
            state.opts = state.opts || {};

            if ( state.opts.noSwitch !== false ) {
                throw path.buildCodeFrameError(
                    `switch-case statement is not avaliable in async for current compiler`
                );
            }
        ...
        visitor: {
            Program: {
                ...
                exit: function (path, state) {
                    ...
                    if ( state.opts.hoistFuns ) {
                        hoistFunction(path, babel);
                    }
            ...
            Function: function Function(path, state) {
                if (path.node.async) {
                    ...
                    // TODO 需要处理下不支持的模式，比如switch里面的break语句
                    path.traverse(warnSwitchCaseVisitor, state);
                }
            }
        }
    };
