...
var _babelPluginTransformRuntimeNebula = require("@ali/babel-plugin-transform-runtime-nebula");

var _babelPluginTransformRuntimeNebula2 = _interopRequireDefault(_babelPluginTransformRuntimeNebula);
...
function preset(context) {
  ...
  var nebulaTargetEnvs = ["common", "ios8", "u4"];

  var loose = void 0;
  var modules = "commonjs";
  var spec = false;
  var enableForOfArray = false;
  var enableGenerator = false;
  var enableTypeofSymbol = false;
  var nebulaTarget = "common";
  var preferExternalPromise = false;
  var requeue = true;
  var ensureExitPromise = false;
  var loosePolyfill = false;

  if (opts !== undefined) {
    if (opts.loose !== undefined) loose = opts.loose;
    if (opts.modules !== undefined) modules = opts.modules;
    if (opts.spec !== undefined) spec = opts.spec;
    if (opts.enableForOfArray !== undefined) enableForOfArray = opts.enableForOfArray;
    if (opts.enableGenerator !== undefined) enableGenerator = opts.enableGenerator;
    if (opts.enableTypeofSymbol !== undefined) enableTypeofSymbol = opts.enableTypeofSymbol;
    if (opts.nebulaTarget !== undefined) nebulaTarget = opts.nebulaTarget;
    if (opts.preferExternalPromise !== undefined) preferExternalPromise = opts.preferExternalPromise;
    if (opts.requeue !== undefined) requeue = opts.requeue;
    if (opts.ensureExitPromise !== undefined) ensureExitPromise = opts.ensureExitPromise;
    if (opts.loosePolyfill !== undefined) loosePolyfill = opts.loosePolyfill;
  }

  loose = (0, _loose.looseFormat)(loose);
  ...
  return {
    plugins: [[_babelPluginTransformEs2015TemplateLiterals2.default, { loose: loose.TemplateLiterals, spec: spec }], _babelPluginTransformEs2015Literals2.default, _babelPluginTransformEs2015FunctionName2.default, [_babelPluginTransformEs2015ArrowFunctions2.default, { spec: spec }], _babelPluginTransformEs2015BlockScopedFunctions2.default, [_babelPluginTransformEs2015Classes2.default, { loose: loose.Classes }], _babelPluginTransformEs2015ObjectSuper2.default, _babelPluginTransformEs2015ShorthandProperties2.default, _babelPluginTransformEs2015DuplicateKeys2.default, [_babelPluginTransformEs2015ComputedProperties2.default, { loose: loose.ComputedProperties }], enableForOfArray && [_babelPluginTransformEs2015ForOf2.default, { loose: loose.ForOf }], _babelPluginTransformEs2015StickyRegex2.default, _babelPluginTransformEs2015UnicodeRegex2.default, _babelPluginCheckEs2015Constants2.default, [_babelPluginTransformEs2015Spread2.default, { loose: loose.Spread }], _babelPluginTransformEs2015Parameters2.default, [_babelPluginTransformEs2015Destructuring2.default, { loose: loose.Destructuring }], _babelPluginTransformEs2015BlockScoping2.default, enableTypeofSymbol && _babelPluginTransformEs2015TypeofSymbol2.default, [_babelPluginTransformRuntimeNebula2.default, {
      "helpers": false,

      "polyfill": true,

      "regenerator": enableGenerator,
      "requeue": requeue,
      "forOfArray": enableForOfArray,
      "nebulaTarget": nebulaTarget,
      "preferExternalPromise": preferExternalPromise,
      "ensureExitPromise": ensureExitPromise,
      "loosePolyfill": loosePolyfill
    }], modules === "commonjs" && [_babelPluginTransformEs2015ModulesCommonjs2.default, { loose: loose.Modules }], modules === "systemjs" && [_babelPluginTransformEs2015ModulesSystemjs2.default, { loose: loose.Modules }], modules === "amd" && [_babelPluginTransformEs2015ModulesAmd2.default, { loose: loose.Modules }], modules === "umd" && [_babelPluginTransformEs2015ModulesUmd2.default, { loose: loose.Modules }], enableGenerator && [_babelPluginTransformRegenerator2.default, { async: false, asyncGenerators: false }]].filter(Boolean) };
}
