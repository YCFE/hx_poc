...
var componentImport = require('./component-import');
var componentParser = componentImport.componentParser;
var generateComponentDep = componentImport.generateComponentDep;
var componentBlockFormat = componentImport.componentBlockFormat;
...
module.exports = function (content) {
  ...
  var defaultLoaders = {
    ...
    js: hasBuble ? (require.resolve('buble-loader') + bubleOptions) : hasBabel ? require.resolve('babel-loader') : ''
  }
  ...
  // 处理 <component> customBlocks
  output += '\nvar componentDeclaration\n'

  // 生成 <component>块的解析结构
  if (parts.customBlocks && parts.customBlocks.length) {
    var componentBlocks = parts.customBlocks.filter(componentBlockFormat)
    var parsedBlockMap = Object.create(null);

    if (componentBlocks) {
      output += 'componentDeclaration = {}\n'
      // 处理
      componentBlocks.forEach(function (block) {
        // 分别处理是否需要懒加载的情况
        var parsedBlocks = componentParser(block);
        parsedBlocks.forEach(function (block) {
          if ( parsedBlockMap[ block.identifier ] ) {
            // 如果发现id重复，编译期间抛错
          }
          parsedBlockMap[ block.identifier ] = block;
        });
      })
    }

    // 输出到 output上
    Object.getOwnPropertyNames(parsedBlockMap).forEach(function (blockName) {
      var block = parsedBlockMap[blockName];
      output += 'componentDeclaration["'+block.identifier+'"] = '+ generateComponentDep(block) +';\n';
  ...
  output += '  /* componentDeclaration */\n'
  // inject componentDeclaration as components properties
  output += '  componentDeclaration'
