  ...
  <div id="app" v-if="options">
    <div class="transfer">
      ...
      <section class="clearfix" v-for="(item, index) in options" :key="index" v-show="isShowSection">
        ...
        <div class="pull-left" @click="productDetail(item)">
          <p>{{ item.name }}</p>
          <p class="font-light-gray">交易金额： <span class="font-red">{{item.money}}</span></p>
          <p class="font-light-gray">{{item.time}}</p>
        </div>
        <div class="pull-right block-right" @click="productDetail(item)">
          <p>{{item.transfer}}</p>
...
<style lang="less">
  ...
  .i-img-red-arrow{
    background-image: url('~@/assets/img/transfer_12.jpg');
  ...
  .ui-checkbox+label {
    ...
    background: url('~@/assets/img/transfer_03.jpg') no-repeat;
    display: inline-block;
    background-size: 38px 38px ;
    font-size: 30px;
  }
  .ui-checkbox:checked + label {
    background: url('~@/assets/img/transfer_10.jpg')  no-repeat;
    background-size: 38px 38px ;
  }
</style>
